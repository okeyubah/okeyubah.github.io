...
particular type. That type is none other than int, a kind of number. Java has other types of data,
like "String", which could be used to hold the teacher's name. If we wanted the teacher's name to
replace their age in our definition of the Teacher class, then our code, above, will need to be
replaced by the following lines:

class Teacher { <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;String name;<br> }

Notice that the word int has now been replaced by String, since the type of name is String, not int.
